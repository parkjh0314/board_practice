   ...
   <form name="searchFrm">
      <c:if test="${not empty deptIdList}">
      	<c:forEach var="deptId" items="${deptIdList}" varStatus="status">
      		...
      		<input id="${status.index}" type="checkbox" value="${deptId}" name="deptId"/>&nbsp;&nbsp;
      	</c:forEach>
      </c:if>
      <input type="text" name="sDeptIdes">
      ...
      <c:if test="${not empty empList}">
      	<c:forEach var="map" items="${empList}">
      		<tr>
      			...
      			<td style="text-align: right;"><fmt:formatNumber value="${map.monthsal}" pattern="#,###"></fmt:formatNumber></td>
