...
<script type="text/javascript">
   ...
   function showWeather() {
	$.ajax({
		...
		type:"GET", // 공공데이터는 POST방식 없음, 무조건 다 GET방식이므로 안써줘도됨. 기본이 GET이니까
		dataType:"XML",
		success: function(xml){
			var rootElement = $(xml).find(":root"); // :root => 문서의 제일 위, 시작점을 말함
			//console.log("확인용 :" + $(rootElement).prop("tagName"));
			// 확인용 : current
			
			var weather = $(rootElement).find("weather"); //rootElement의 하위 태그에서 weather태그를 찾음
			var updateTime = $(weather).attr("year")+"년 "+$(weather).attr("month")+"월 "+$(weather).attr("day")+"일 "+$(weather).attr("hour")+"시 ";
			//console.log(updateTime);
			
			var localArr = $(rootElement).find("local"); //local태그여러개이므로 배열로받아옴
	       ...
	       //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	            ...
	            for(var i=0; i<localArr.length; i++){
					var local = $(localArr).eq(i); // 한개태그를 가져올때는 .eq를 사용
				/* 
           		...
           		*/
					...
					html += "<td><img src='/board/resources/images/weather/"+icon+".png'/>"+$(local).attr("desc")+"</td>";
       ...
       // ========== XML을 JSON으로 변경하기 ============= //
			var str_jsonObjArr = JSON.stringify(jsonObjArr); 
       							// JSON.stringify() : JSON객체를 String(문자열) 타입으로 변경해주는 것
       		$.ajax({
       			url:"<%=request.getContextPath()%>/opendata/weatherXMLtoJSON.action",
       			type:"POST", //get으로 보내면 데이터가 너무 많아서(95개) 오류남
