...
public class MemberVO {
	...
	private int pwdchangegap;			// 마지막으로 암호를 변경한후 지난 개월수 (select용)
	private int lastlogingap;			// 마지막으로 로그인을 한 후 지난 개월수 (select용)
	
	// ===#138. 먼저 답변글쓰기는 일반회원은 불가하고 직원(관리자)들만 답변글쓰기가 가능하도록 하기위해 tbl_member 테이블에 gradelevel이라는 컬럼을 추가해야한다.
