...
public class BoardDAO implements InterBoardDAO {
	   ...
	   //                       스프링 컨테이너에 담겨진 의존객체를 주입할때 '타입'을 찾아서 연결(의존객체주입)한다.
...
//	private SqlSessionTemplate abc;
	...
	@Resource
	private SqlSessionTemplate sqlsession3; //로컬 DB의 hr에 연결
	// Type 에 따라 Spring 컨테이너가 알아서 root-context.xml 에 생성된 org.mybatis.spring.SqlSessionTemplate 의 bean 을  abc 에 주입시켜준다. 
	// 그러므로 abc 는 null 이 아니다.
	
	// spring_test 테이블에 insert하기
	@Override
	public int test_insert() {
		int n = sqlsession.insert("board.test_insert"); //abc.insert(mapper파일(xml)의 namespace명.mapper파일 내 sql문의 아이디값)
		return n;
	}
	...
	public List<Map<String, String>> test_employees() {
		...
		// board.xml 써도 되지만, 헷갈리지 않게 새 mapper를 만듦(hr.xml)
		// List로 넘겨줄거니까 .selectList()
		return empList;
	}

	
	// ==== #38. 메인페이지용 이미지 파일을 가져오기 === //
	@Override
	...
	// === #60. 페이징 처리를 안 한 검색어가 없는 전체 글목록 보여주기 === //
	@Override
	...
	// tbl_board테이블에 commentCount 컬럼의 값을 1 증가(update)
	@Override
	...
	// === #116. 총 게시물건수(totalCount) 구하기 - 검색이 있을때와 검색이 없을때로 나뉜다. ===
	@Override
	...
	// === #130. 원게시물에 달린 댓글들을 페이징처리해서 조회해오기(Ajax 로 처리) === //
	@Override
	...
	// === #134. 원게시물에 달린 댓글들의 totalPage 알아오기 (Ajax 로 처리) === //
	@Override
